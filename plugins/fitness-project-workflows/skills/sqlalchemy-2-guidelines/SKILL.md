...
Use this skill for SQLAlchemy work. The stable baseline is SQLAlchemy **2.0.52** and its official documentation.
...
- Respect the SQLAlchemy version pinned by the target project. Do not recommend APIs unavailable in that version.
...
- Keep application-specific conventions in the project guidance; this skill supplies SQLAlchemy guidance, not an application architecture.
...
- Make persistent schema changes with Alembic migrations; keep model and migration changes aligned.
- Review autogenerated migrations before applying them, including constraints, indexes, nullability, foreign keys, and data-migration needs.
- Do not create, alter, or drop application tables at runtime with `create_all()` or `drop_all()`.
